...
main(int argc, char *argv[])
{
    ...
    //CUPTI library don’t support source level analysis for TURING+ chips.
    if (((prop.major > 5) && (prop.major < 7)) || ((prop.major == 5) && (prop.minor == 2)) || ((prop.major == 7) && (prop.minor <= 2))) {
        CUPTI_CALL(cuptiActivityEnable(CUPTI_ACTIVITY_KIND_PC_SAMPLING));
    } else {
        printf("sample is waived on this device, pc sampling is supported on devices with compute capability 5.2 or 6.0 and CC <= 7.2 i.e supported till volta\n");
        return 0;
    }
